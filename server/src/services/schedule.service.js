...
async function login(req) {
  ...
  storePWD = pwd
  if(pwd){
    return { status: isPwdValid ? 200 : 401, json: isPwdValid ? {msg: "login success", token: jwt.sign({ pwd }, process.env.TOKEN_SECRET, { expiresIn: 60*5 })} : "login failed" };
  }
...
async function userLogin(req) {
  ...
  if (isUserRegistered) {
    ...
    try {
      ...
      return { status: 200, json: {msg: "login success", token: jwt.sign({ userHash }, process.env.TOKEN_SECRET, { expiresIn: 60*5 })} };
    } catch (error) {
      console.error(error);
    ...
    try {
      ...
      return { status: 200, json: {msg: "registration success", token: jwt.sign({ userHash }, process.env.TOKEN_SECRET, { expiresIn: 60*5 })} };
    } catch (error) {
      console.error(error);
